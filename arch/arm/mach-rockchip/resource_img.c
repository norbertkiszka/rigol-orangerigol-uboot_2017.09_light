...
static int resource_image_check_header(const struct resource_img_hdr *hdr)
{
	...
	if (ret) {
		printf("bad resource image magic: %s\n",
			   // Norbert: gcc 12 gives a error (Linaro gcc -6.3.1-2017.05 does not): error: the comparison will always evaluate as ‘true’ for the address of ‘magic’ will never be NULL [-Werror=address]
